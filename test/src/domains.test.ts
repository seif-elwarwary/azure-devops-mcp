...
describe("DomainsManager: backward compatibility and domain enabling", () => {
  ...
  describe("constructor", () => {
    ...
    it("enables all domains when undefined is passed as argument", () => {
      ...
      expect(enabledDomains.size).toBe(11);
      expect(Array.from(enabledDomains).sort()).toEqual(["admin", "advanced-security", "core", "pipelines", "release", "repositories", "search", "test-plans", "wiki", "work", "work-items"]);
    });
  ...
  describe("string input handling", () => {
    ...
    it("Error and enables all domains when an invalid domain name string is passed", () => {
      ...
      expect(errorSpy).toHaveBeenCalledWith(
        "Error: Specified invalid domain 'invalid-domain'. Please specify exactly as available domains: admin, advanced-security, pipelines, release, core, repositories, search, test-plans, wiki, work, work-items"
      );
    });
  });
  ...
  describe("getAvailableDomains method", () => {
    it("returns the full list of available domains", () => {
      ...
      expect(availableDomains).toEqual(["admin", "advanced-security", "pipelines", "release", "core", "repositories", "search", "test-plans", "wiki", "work", "work-items", "mcp-apps"]);
